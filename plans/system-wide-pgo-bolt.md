...
- **Project state:** active; Phase 0 and Phase 1 are complete and Phase 2 implementation is complete at 14/14. Historical Candidate A is clean; the former Candidate-B freeze was invalidated by later source changes. Candidate C is the next frozen authorization source; authorization remains none until its detached index verifies 0/0/0. Phase 2 is scope-frozen: no new subsystem, optimization-policy axis, evidence category, or broad refactor may be added unless an existing required gate exposes a reproducible blocker that cannot be fixed within the current architecture.
