...
- **Rejected predecessor boundary:** the prior authoritative retry at `eb0cd18b30f44bd6e7b30ef0e1499be302718730` is retained as non-authorizing evidence because its pidfd unit-test expectation was stale (`1` production descriptor, not `2`). The correction is test-only and does not indicate a production framework failure; a fresh exact-source boundary is required before detached authorization.
