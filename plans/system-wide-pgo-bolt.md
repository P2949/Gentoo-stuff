...
- **Current execution frontier (supersedes the predecessor narrative below):** the jsonschema prerequisite and terminal post-jsonschema checkpoint remain complete historical authorities; no prerequisite replay is authorized. Candidate `0a870c01b09d8e662ba21fed5a6061f8f9d9b398` is retained as superseded non-authorizing evidence: its 91/95 live run used the predecessor framework, and review found a stale evidence-index marker plus an evidence-root lookup defect. The bounded successor correction now binds `journal.child_contract.evidence_output_root`, enforces the trusted production reports root, and adds a non-default-root regression. After the marker is refreshed, freeze one exact successor, install/materialize it, rerun the complete authoritative live boundary from the beginning, then without source or plan changes run the supervised production sample transaction, generate all eleven component states, and capture and independently verify the detached index at 0/0/0. No production sample transaction, component-state namespace, detached index, optimization generation, package rebuild, or Phase 3 inventory has been authorized. The latest live VDB observation is non-frozen and stale after the interrupted wave. Boot, kernel, EFI, initramfs, and bootloader mutation remain prohibited.
