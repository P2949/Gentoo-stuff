...
- **Current execution frontier (supersedes the predecessor narrative below):** `6de56e6` removes the invalid zero-delta finalization path that published `offline-restore-proven` without an actual binary restore. Empty-delta failure handling remains limited to exact no-op rollback/VDB verification; terminal `offline-restore-proven` still requires an exact restore CPV and the established verifier receipt. The live pre-dependency checkpoint `checkpoint-pre-candidate-a-deps-20260905T000008Z` remains the terminal restore-proven baseline with pending/unknown/failed totals `0/0/0`. No prerequisite package transaction has run, and none of the seven Phase-2 acceptance boxes closes. The correction is locally syntax-checked and its prerequisite unit suite passes 105 tests with four intentional host-capability skips; a complete recovery-suite run remains environment-sensitive and is not treated as green evidence.
