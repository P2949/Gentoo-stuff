...
The superseding authoritative fixture passes 19/19 cases with zero failures. It additionally proves an existing Portage profile/package `post_src_install` hook is preserved, runs before the optimization hook, and is not recursively captured if the user bashrc is sourced again. Root-owned evidence is `/var/lib/gentoo-optimization/reports/phase-2-dispatcher-hook-chain-20260712` (manifest SHA-256 `0db50cbfe5232d9c9ca5c3874198de483c407a4081dfbc2bcbbeee926dc0d1b7`); component state is `/var/lib/gentoo-optimization/state/project/phase-2-dispatcher.json` (SHA-256 `af388f101eb9ac16f5fabcdcc79f54531e7166b06296ccc0b16c09ab295bb288`). The tested `portage/bashrc` SHA-256 is `9c8eedc4760d82a5116859abb6be22d9a701fd34d0756495a0758f2464887414` and the fixture SHA-256 is `d21aee1119e932ea8bdb807d6b9693c8634b33733a6649fa671683586d46c09f`. Strict live policy validation still covers 147 atom/environment pairs over 1,217 installed CPVs. The earlier 18-case evidence remains preserved but is superseded. `active_generation` remains null and `inventory_frozen` remains false.
...
### 2026-07-15 pre-activation remediation checkpoint

- The repository implementation now writes only `sample.prof`, publishes it transactionally under the full generation/inventory lock hierarchy, validates it with sample-aware `llvm-profdata show --sample`, and records the exact mapped binary build ID, `.text` SHA-256, full-file identity, perf identity, conversion command, and immutable conversion log. The dispatcher has only the `-fprofile-sample-use`/`-fsample-profile-use-profi` sample consumer; the IR consumer remains separate.
- Mapping-input and consumer-build fingerprints are now distinct identities. The real Portage fixture derives the mapping fingerprint from an observed mapping-ready receipt, derives the consumer fingerprint from the exact ordinary receipt plus the deterministic sample-use transformation, binds both identities into the validator sidecar, and refuses completion unless the authoritative sample-use receipt reproduces every predicted flag, feature, USE, package-environment, and extra-build axis exactly.
- Production profile locks and immutable profile artifacts are root:`portage` readable without becoming Portage-writable: the runtime lock directory is `0750`, stable locks and profile/metadata/manifest files are `0640`, the conversion log and perf input are `0440`, and the preserved mapped input is `0550`. The fixture separately records the validator running as the `portage` UID/GID under `FEATURES=userpriv`.
- Repository-only syntax, ShellCheck, type, unit, lock-contention, transactional publication, and reduced real perf-to-`llvm-profgen` checks pass. The four checkboxes below intentionally remain open until the reviewed framework is installed from a clean commit and the complete disposable Portage mapping -> perf -> conversion -> validation -> generated `package.env` -> sample-use -> runtime/tamper gate passes on the live host.
...
The 2026-07-15 privilege-metadata audit supersedes the original staging-order proof. Both deployment and rollback now stage in the only safe order: private copy, ownership, every hardlink, ordinary/capability xattrs, and final setuid/setgid mode last, followed by completed-inode-group verification and an inode fsync before rename. Independent root tests cover `04755`, `02755`, `06755`, user xattrs, `security.capability`, hardlink groups, and rollback. Static PIE is a distinct fourth real BOLT class: `ET_DYN` without `PT_INTERP` but with `DF_1_PIE` is an executable, while an `ET_DYN` object without the PIE flag remains a DSO. The real four-class perf/BOLT fixture and the root staging fixture pass. Production framework/project/generation locks are root:`portage` `0640` below a root:`portage` `0750` runtime directory; a live disposable Portage receipt proves compile phases run as `portage` while `pre_src_install`, `src_install`, `post_src_install`, and the final install-QA hook run as root, so BOLT does not require disabling `userpriv`.
...
The framework publisher is now candidate-complete and crash-consistent: Portage, overlay, helpers, schemas, QA logic, generated policy, and the manifest live inside one immutable candidate. First migration installs a fail-closed Portage guard before its fsynced activation journal; normal upgrades change behavior only through the atomic `framework-current` rename. Each installed bashrc embeds and exports its exact candidate target, and stable shell/Python/QA bootstraps honor that pin, so a build begun on generation A cannot call generation B after an upgrade; re-sourcing another generation fails closed. Fixed bootstrap bytes are an invariant upgrade ABI and a changed renderer is rejected before external publication. The hermetic fixture covers SIGKILL on both sides of first activation and upgrades, same-generation re-source, a real old-bound process across activation, cross-generation rejection, and incompatible bootstrap migration. This remains a repository/framework claim until the clean live install and combined host gate below pass.
