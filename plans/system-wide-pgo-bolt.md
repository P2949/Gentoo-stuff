...
## Progress summary
- 2026-09-21: The live instrumentation census required by the P0 cleanup completed against a fresh VDB/CONTENTS-owned artifact census: 16,731 regular ELF records were inspected and 3,628 instrumented runtime ELFs were found across 224 CPVs, plus two non-ELF firmware/readelf failures. Shared contamination includes util-linux, PAM, OpenRC, GTK/GStreamer, NSS, CUPS, ncurses, iptables, and many other system components; the immutable census is `/var/lib/gentoo-optimization/reports/live-instrumentation-20260921-p0.json` (SHA-256 `6268146f3b192a716dc85b5b9ed9f201ee2d2d86cdc4876b43350d2009a4220b`). New source tooling `scan-live-instrumentation.py` records ELF markers, build IDs, VDB identity hashes, and fail-closed tool errors. No new generation wave is authorized until these installed artifacts are reconciled.
- 2026-09-21: Root cause was confirmed beyond package-specific bindings: `run-profile-wave.py` removed `LLVM_PROFILE_FILE` from its administrative environment, causing any instrumented helper to fall back to relative `default.profraw`; long-lived Rust/Cargo flags could independently preserve old generation paths. The runner now exports `/dev/null` for helpers, the Portage dispatcher scrubs inherited Rust profile flags in every lane (including `off`), and the desktop/user-session environment now defaults to `/dev/null` while reviewed workloads override it explicitly. Focused dispatcher and profile-wave regressions pass; the live framework was republished from commit `2ba00dd`.
- 2026-09-21: Traced recurring LLVM `default.profraw` permission errors beyond the desktop screenshot binding to stale Rust profile flags inherited by long-lived Portage/Cargo environments. The shared `portage/bashrc` dispatcher now removes inherited `-Cprofile-generate=`/`-Cprofile-use=` destinations from `RUSTFLAGS`, `CARGO_BUILD_RUSTFLAGS`, and encoded Cargo flags before applying the authenticated lane identity; a focused regression proves an old generation spool can never survive Rust mode selection. This prevents stale root-owned paths such as the 2026-09-18 Rust spool from being reused by unrelated packages.
- 2026-09-21: Fixed the live desktop profile-environment leak: the Hyprland Print binding now invokes `grimshot` with `LLVM_PROFILE_FILE=/dev/null`, so inherited generation destinations cannot make screenshot helpers emit `default.profraw` permission errors. A controlled instrumented probe reproduced the old root-owned-spool failure and completed cleanly with `/dev/null`; the original binding was preserved at `/home/p2949/.config/hypr/hyprland.lua.codex-backup-20260921`. This is a runtime-environment remediation and does not alter authenticated profile-wave destinations.
...
- 2026-09-21: Phase-4 workload accounting now distinguishes smoke from representative training at commit `3ba3e1b`: generated recipes carry explicit purpose/state (`smoke-ready`, `needs-training-workload`, `terminal-workload-exclusion`), and coverage emits an independent `representative_training_coverage_pass` gate instead of treating `--help` smoke as training. The focused Python compilation boundary passed and the framework-installer regression completed with its snapshot/publication/rollback fixture passing; commit pushed to `feat/system-wide-pgo-bolt`. No broad profile-use wave was resumed.

- 2026-09-21: Regenerated the live VDB inventory after the strict-parser fix as `/tmp/phase3-live-strict-20260921.json`: 1,307 packages, 684,782 owned paths, 80,079 owned directories, zero unresolved directories, and 764,861 source CONTENTS records (`dir=80,079`, `obj=648,700`, `sym=36,082`), SHA-256 `5ed08c15f7a2a4cf64df7825aea860e5c90d5854c640e4240808fa2cbd0553c7`. The extended frozen-inventory verifier independently passed the source-accounting totals. This is candidate evidence only; mutation-policy/source review and framework activation remain pending.

- 2026-09-21: Closed the next Phase-3 authority defects at commit `0921537`: live inventory generation now uses the shared strict CONTENTS parser and fails closed on malformed/unsupported VDB records, while recording source-record counts; mutation-policy verification independently reopens the source kernel/lifecycle classification, requires complete CPV coverage, and preserves the classifier's concrete reason code in policy triggers. Added focused malformed-CONTENTS and source-bound mutation-policy regressions; pushed to `feat/system-wide-pgo-bolt`. The fresh live candidate still requires regeneration and must not be treated as final authority until its unresolved source/lifecycle records are resolved.

- 2026-09-21: Deep integration review found the baseline profile-use wave ended without a wrapper return marker while OpenJDK bootcycle compilation was active; no emerge process remains. The immutable log `/var/lib/gentoo-optimization/reports/userspace-baseline-update-20260921-r1.txt` is preserved (SHA-256 `40b648faad88fb6c4b9d7aa6b6f58cf7a1f94ffaddb1ed90b5c471b0e6acfbe2`, 81,235 lines) as an interrupted, non-authoritative attempt. Its package-local failures remain preserved: libde265/libmediainfo/mesa ABI guard losses, Hyprland dispatch failures, git-lfs fetch, ncspot cargo build, and cups dispatch refusal. A fresh userspace pretend now resolves 50 operations; no retry was started from the stale candidate.

- 2026-09-21: Repaired two directly reproduced producer/consumer schema breaks at commit `72ff340`: carry-forward producer and verifier now require identical `package_env_content` and `build_controls` fields, and the canonical dispatcher record now publishes schema-v2 framework and compiler identities required by profile-use receipt v2. Focused carry-forward, dispatcher, and Python compilation tests pass; the commit was pushed to `feat/system-wide-pgo-bolt`. The deeper Phase-3 mutation-policy/schema revision, canonical set derivation, authoritative CONTENTS parser, and Phase-5 toolchain bootstrap remain next implementation work; no profile-use wave was resumed.

- 2026-09-21: Package-state classification now accepts a generation-bound canonical mutation-policy artifact and carries its exact decision/triggers/evidence into every package record; the legacy atom-set input remains compatibility-only. Kernel-policy classification now refuses missing exact repository ebuild evidence as `pending-lifecycle-review/source-unavailable` instead of searching another repository or defaulting to userspace, and parses whitespace-bearing CONTENTS paths without lossy splitting. Focused Python compilation passes; commit `ec59f92` pushed. The full v6 state-schema migration and regenerated 1,307-package policy remain pending.
...
- 2026-09-21: Added generation-bound canonical mutation-policy production and independent verification. Every frozen CPV must now resolve to exactly one `userspace` or `kernel-policy-exclusion` decision with source classification hashes, repository/ebuild provenance, triggers, and evidence; unresolved lifecycle reviews refuse publication. Synthetic generator/verifier integration passes; commit `f9367a1` pushed. The live 1,307-package policy has not yet been published because its source classification must be regenerated against the final candidate inventory.

- 2026-09-21: Applied the fail-closed kernel classifier to the 1,307-package candidate. It produced 8 concrete forbidden-artifact exclusions, 981 userspace transactions, and 318 pending lifecycle/source reviews; canonical mutation-policy publication correctly refuses this unresolved set. The pending count includes source-less generated/account packages and must be resolved by exact installed/next-build provenance before Phase-3 authority can close. CPV package-name derivation now uses Portage/VDB evidence for zero-version packages; commit `8740d6a` pushed.
...
- 2026-09-21: Added immutable installed-versus-next-build provenance collection and independent verification. Applying it to the retained 1,307-package candidate exposed current VDB drift (`app-accessibility/at-spi2-core-2.60.6` is no longer installed), so that candidate was rejected rather than reused. A fresh live inventory attempt found 1,307 packages, 684,782 owned paths, 80,079 directory records, and 44 unresolved directories; the generator now records `unresolved_directories` explicitly instead of omitting the field. Commit `0c49c23` pushed; authoritative regeneration remains pending review of those 44 directories and current package identities.

- 2026-09-21: Extended kernel-policy and provenance lookup to use Portage's exact repository API rather than only guessing repository directory paths. Against the fresh root-owned inventory and directory review, exact classification improved to 1,286 userspace transactions, 8 concrete exclusions, and 13 pending records. The remaining pending set is now narrow and explicit: two Maya overlay CPVs, Ruby/tree-sitter/GitHub CLI/Clang runtime/Maya/Firefox source-unavailable identities, one missing dracut source, one virtual secret-service record, and three boot-lifecycle marker reviews. Commit `9560457` pushed; no profile-use wave resumed.

- 2026-09-21: Lifecycle keywords are now review triggers rather than automatic exclusions. The classifier requires concrete forbidden phase mutation in the exact ebuild before assigning kernel-policy exclusion; `efibootmgr`, `uefi-mkconfig`, and `systemd-utils` therefore remain userspace transactions when their current ebuilds only install tools/log guidance. Fresh classification is 1,288 userspace, 9 concrete exclusions, and 10 pending source-unavailable records. Focused classifier and Python compilation pass; commit `3568792` pushed.

- 2026-09-21: Preserved the fresh current-VDB candidate as root-owned generation evidence at `/var/lib/gentoo-optimization/generations/phase3-live-current-20260921d/`: inventory SHA-256 `52414cbb72d5f6be82ebfecb37b8a47976eca5fd4a8c0b6c75cac5d3ba9a46ab`, directory-review SHA-256 `ecaee66fb3a09e2aa5fe51ea5789c5048e8a60d4a510816fa976b608aea50564`, kernel-classification SHA-256 `96df21bac5be770765503ad78ea801f09e92de349319634175072c11ef1e1f56`, and provenance SHA-256 `1e5a3f6ab74debbac7fbd2628055fab7081e2e4d66bcd7d7df04ec3397cb45e7`. Independent provenance verification passes for all 1,307 CPVs; this remains candidate evidence because 10 exact source/lifecycle records are pending.
...
- 2026-09-21: Materialized a fresh 1,304-record generated policy from `/tmp/phase3-current/pgo-policy-bindings.json` (package.env SHA-256 `8c3724637cbf3cffd86ea67c812f7acc9134212b79735ccd97e6c8b622c6c775`) and staged it as candidate evidence. The installer correctly refused the accompanying `/tmp/live-inventory-20260921.json` because its directory records were not strict authoritative generation inventory (`invalid owned directory`); no framework activation occurred. The candidate policy and refusal remain preserved for regeneration using the reviewed directory-resolution path.
...
### 2026-09-21 — baseline candidate strict inventory and policy-authority refusal

A root-authority regeneration from the live VDB and the merged frozen-directory review produced the strict candidate inventory `phase3-live-candidate-20260921-baseline`: 1,307 packages, 684,698 owned paths, 80,075 owned directories, and zero unresolved directories. The candidate was staged under a versioned root-owned generation and checked through the production bootstrap installer. The first strict check rejected the stale pre-update policy because seven generated package.env atoms were absent from the authoritative inventory (gzip, unicode-data, deno-bin, both Ayatana successors, libltdl, and xfsprogs); no activation occurred. A filtered policy was then checked only to verify the next gate and was correctly refused at the active framework identity boundary. The filtered tree is not authoritative and was not activated. The required follow-up is to regenerate policy bindings and fingerprints from the same 1,307-CPV inventory before any framework publication or further profile-use deployment.

### 2026-09-21 — fresh 1,307-CPV baseline policy activated

The live VDB successor set was regenerated into a fresh candidate: 1,307 package records, 538 exact PGO-lane fingerprint inputs with zero materialization failures, successor lane manifest SHA-256 `8afc265349ff8de4800709158cd72ef1bc9c5cb6bbb92100f370de316f9a8bf7`, policy-binding SHA-256 `73da7ca64007457415aaad4e6275fa18bc8766315dd71edbca7b1a646b79a047`, and generated-policy tree identity `d48a8065d084c21de0578e78fb42b7a89685b15c23d74f4c83a01431cb9d4d29`. The normal root-owned bootstrap installer published the candidate and activated framework `framework-969a8348488aa2c91493d68a2c5e12d2852013e4dcca265647e6b24847c3c2fd`; the active package.env now selects `gzip-1.15`, `libayatana-appindicator-0.6.0`, and `libayatana-indicator-0.9.5`, with the stale predecessor assignments absent. An independent strict installer check passed against the fresh versioned frozen inventory and generated-policy tree. No boot, kernel, EFI, initramfs, firmware, or ABI guard boundary was bypassed.

### 2026-09-21 — baseline maintenance retries after policy activation

After the fresh framework activation, `app-shells/bash-9999` was retried under its exact maintenance `optimization-off` assignment. The package merged successfully; its upstream test suite reported 81/85 tests succeeded (known environment-sensitive failures) but Portage completed the merge and install-QA path without a profile-use claim. `media-libs/libde265-1.1.3` was retried after enabling the prior decoder/encoder/tool USE set to preserve the installed public ABI. The ebuild still produced only 106 exported symbols versus 877 in the installed 1.0.16 provider, and the ABI guard correctly refused admission. The complete retry remains preserved in `/var/tmp/gentoo-portage-build/portage/media-libs/libde265-1.1.3/temp/build.log`; this is an upstream/package ABI-compatibility failure requiring a reviewed source/version resolution, not a guard bypass.
...
### 2026-09-21 — baseline drift reconciliation after nine successor merges

The live CPV audit correctly detected that the package count remained 1,307 while nine exact identities had advanced beyond the previously authorized inventory (`at-spi2-core` and eight Python packages). A fresh root-authority inventory was regenerated with 684,772 owned paths, 80,075 directories, and zero unresolved directories. Current lane/fingerprint inputs were regenerated with 538 successful PGO-lane materializations, fresh policy bindings, and generated-policy identity `44ac94f7bed1c84c4438f96fd306b9fd56a51d2cd76cd8ab729d9fabbd252278`. The root-owned installer published framework `framework-429649174642df29091bbb9fec37d66070b342043a71d10930fbeb602285b71c`; independent strict framework verification passed. Runtime lock metadata was cleared only after confirming no coordinator process held the locks, then generation authorization activated and independently verified `phase3-live-candidate-20260921-current` with inventory SHA-256 `10ce5a8c24c1acdfa9dcbdefc6486bcf7628660cd36067d5e2ce62edf7edd8aa`. No package, boot, kernel, EFI, initramfs, or firmware bypass was used.
...
### 2026-09-21 — libgcrypt profile-use TLS probe remediation staged

The preserved `dev-libs/libgcrypt-1.12.4` failure was narrowed to the 32-bit configure capability probe: Clang's profile-use freshness/unprofiled diagnostics caused the probe for `__thread` support to report `no`, which then triggered libgcrypt's FIPS TLS hard error. A package-local environment fragment now demotes only those two profile diagnostics from errors for libgcrypt; FIPS, multilib, and ABI-guard policy remain unchanged. The generated-policy tree was materialized, and the framework installer correctly refused publication until this source change was committed, preserving the source-authority boundary. A fresh framework publication and exact libgcrypt retry remain required.

### 2026-09-21 — libgcrypt workaround published and TLS probe verified

After commit `0e28893`, the root-owned framework installer published framework `framework-7a44a2741312c319a5023c1ad7c87d2c29f098b3c911da47275e243219e13f95`; the active `/etc/portage/package.env/10-no-polly` now contains the reviewed libgcrypt assignment. An isolated 32-bit Clang probe using the retained profile, `-Werror`, and only the two package-local diagnostic demotions completed with `tls_probe_rc=0`, proving the TLS capability probe no longer fails for the diagnosed reason. The old libgcrypt dispatcher remains generation-stale and is intentionally not reused; a fresh current-generation profile wave and exact profile-use retry are still required before claiming package success.

### 2026-09-21 — libgcrypt generation retry succeeded; ebuild identity drift refused use publication

The corrected one-package generation wave for `dev-libs/libgcrypt-1.12.4` passed storage, authority, exact-CPV pretend, readiness, and controlled Portage execution. Both 32-bit and 64-bit configure probes reported `__thread` support, the package merged through install-QA and the exported-ABI guard, and the receipt verifier passed. LLVM 22 merged the receipt-bound payloads with evidence digest `f6164c3588f37784ba6a333f276d74aee4fbf96cb75f2361398dddcaef03ca4e`. During the post-merge identity review, Portage refreshed the VDB ebuild copy at `2026-09-21 11:26:47` and its SHA-256 was `3a4d78b3cccd9dbe23afd88dff6f31d2507a83408cb5612d0063dcb732061649`, differing from the pre-transaction fingerprint input. The validated profile and raw evidence are retained, but dispatcher publication was deliberately refused; the current generation must be re-inventoried/re-fingerprinted before any profile-use claim.

### 2026-09-21 — libgcrypt current-identity retraining completed

The post-merge VDB identity was re-materialized (`8c02c0ff3b6dd8b70956edb993ee61d4ad007642e75ec3002a188fa0be309697`) and bound into a new generation identity tree. A second exact generation wave under `phase3-live-candidate-20260921-libgcrypt-final` completed with the corrected fingerprint, passed receipt verification, and merged LLVM 22 evidence digest `0de78c760230a9a45d5af0f7ef6fb5911c7b0cb35c3e4aec7322c4c7095a6d82`. The current VDB ebuild SHA-256 remained `3a4d78b3cccd9dbe23afd88dff6f31d2507a83408cb5612d0063dcb732061649`. The validated profile is now ready for exact dispatcher publication; active package policy publication is being completed through the authenticated framework transaction.
...
### 2026-09-21 — restored package-owned runtime directories and activated reviewed generation

The four package-owned runtime directories were absent from the live filesystem, so the retained reviewed metadata was not sufficient for strict inventory authority. They were recreated with their previously reviewed ownership and modes: `/var/spool/cups/tmp` 0:7/0755, `/etc/X11/xinit/xinitrc.d` 0:0/0777, `/var/spool/cron/lastrun` 0:0/0750, and `/var/spool/cron/crontabs` 0:460/01730. A root-run inventory regeneration then produced 1,307 packages, 684,772 paths, 80,075 directories, zero unresolved directories, and inventory SHA-256 `0bca908fccf93e06081ed6aa9ef8b89675e53459ca560cf8e503ed273004d73d`. `frozen-inventory.py`, strict installer publication/check, and generation authority activation all passed for `phase3-live-candidate-20260921-restored`; no boot, kernel, EFI, initramfs, or firmware state was touched.

### 2026-09-21 — narrow CMake profile-use probe remediation staged

The preserved `dev-build/cmake-4.3.5` failure was reopened from its retained `CMakeError.log`. The C++ try-compile object and link commands completed under `clang-ir-use`; the configure probes then execute those profile-use test binaries, which attempt incidental LLVM runtime profile output and report failure while the authenticated profile is being consumed. A CPV-specific `cmake-profile-use.conf` now sets only `LLVM_PROFILE_FILE=/dev/null`; no profile-use flags, ABI policy, or global runtime behavior were changed. The exact policy stack is reviewed by `check-package-env-duplicates.py`. A fresh exact-generation dispatcher/profile transaction is still required before this remediation can be called successful; the prior failed attempt remains immutable.

### 2026-09-21 — CMake carry-forward decision

The retained CMake profile was evaluated with `carry-forward-profiles.py` against `phase3-live-candidate-20260921-restored`. The decision is explicitly `retrain`, not carry-forward: the corrected target package.env identity contains the new exact CMake probe policy while the retained source identity has no equivalent content binding. The immutable decision is root-owned at `/var/lib/gentoo-optimization/reports/profile-carry-forward-cmake-20260921.json` with source identity `47cf5296901803a43956580bd6f9455e283d6a31f33c1d17335e203da387cc8d` and target identity `f936a0206f63c9d287a0584c3ca942125e993ab69e8d56c3870df55dd8064e11`. The stale profile was not reused and the prior failed CMake attempt remains preserved.

### 2026-09-21 — CMake generation retraining completed; exact profile-use remains a package-local configure failure

`dev-build/cmake-4.3.5` was retrained from the corrected `phase3-live-candidate-20260921-restored` identity rather than carrying forward the stale profile. The controlled generation wave completed with 21,438 receipt-bound LLVM raw payloads, merged profile evidence digest `bb76e1512f0e78f815096dacd01f6e839c6c40ab39139cd6d2c44bbfa5c14d74`, and a new exact dispatcher bound to repository `gentoo` and ebuild SHA-256 `e22daf631445f556b174437667c805ce65e30504904f1ac3872ee57698e9ab16`. The first exact-use attempt correctly refused because its merge-evidence sidecar was unreadable to the Portage build user; that immutable refusal was repaired by publishing a readable root-owned sidecar and rebinding the authority locks. The subsequent exact `=dev-build/cmake-4.3.5::gentoo` transaction consumed the dispatcher and reached configure, where CMake's own C++11 probe again reported `std::unique_ptr` unavailable under profile-use. Receipt `/var/lib/gentoo-optimization/reports/phase3-cmake-profile-use-final-v4-receipt.json` and log remain non-authoritative failed evidence. No unchanged retry or global profile weakening was performed; the remaining action is a narrower CMake configure/probe remediation or explicit retrain decision.

### 2026-09-21 — CMake profile-use probe diagnostic demotion did not resolve configure failure

The package-local CMake profile-use dispatcher was extended with `-Wno-error=profile-instr-out-of-date` for compiler probes only, then republished through the authenticated framework and retried once through the exact runner. The try-compile commands now visibly contain the demotion and still report a failed `make_unique`/`unique_ptr` capability result despite successful compilation diagnostics. The transaction again stopped in CMake configure before merge; receipt `/var/lib/gentoo-optimization/reports/phase3-cmake-profile-use-final-v5-receipt.json` and log are preserved. This remediation is therefore not sufficient; the package remains a narrow `correctness-failure-after-remediation` candidate pending direct probe/link-result diagnosis. No unchanged retry, profile bypass, or global flag change was performed.

### 2026-09-21 — CMake exact profile-use transaction succeeded after warning-sensitive probe fix

The CMake feature-check implementation treats any warning emitted by a try-compile as a failed compiler capability. The prior package-local `-Wno-error=profile-instr-out-of-date` was therefore insufficient because it still emitted the warning. The dispatcher was corrected to use `-Wno-profile-instr-out-of-date` only for CMake's profile-use configure probes, republished through the authenticated framework, and retried through the exact `=dev-build/cmake-4.3.5::gentoo` runner. The configure checks now report `make_unique - yes` and `filesystem - yes`; the package rebuilt, installed, passed install-QA/ABI checks, and receipt verification passed at `/var/lib/gentoo-optimization/reports/phase3-cmake-profile-use-final-v6-receipt.json`. This is Clang IR profile-use evidence only; no BOLT claim is made.

### 2026-09-21 — exact-generation doas profile-use rebuild completed

`app-admin/doas-6.8.2` was retrained under `phase3-live-candidate-20260921-restored` after the corrected identity model rejected its retained predecessor profile. The controlled Clang IR generation wave completed, the profile was merged and validated against the live ebuild SHA-256 `70a076aeb28d3e455f76e1cb6a2c5286f9ba557a603c69a5848e3a77472da40f`, and a dedicated exact dispatcher was published. After binding that dispatcher through the authenticated framework, `run-profile-use.py` completed `=app-admin/doas-6.8.2::gentoo` successfully; independent receipt verification passed at `/var/lib/gentoo-optimization/reports/phase3-doas-profile-use-restored-receipt.json`. This is Clang IR profile-use evidence only; no BOLT claim is made.
...
### 2026-09-21 — exact profile-use runner now supplies an authenticated dispatcher handoff

The production profile-use runner previously verified an exact dispatcher but relied on a separately maintained per-CPV `package.env` assignment to make Portage consume it. That left the dedicated runner incomplete for the remaining package wave. `run-profile-use.py` now requires the adjacent root-owned immutable dispatcher environment and passes a one-shot handoff variable to Portage. Installed `portage/bashrc` validates the handoff path, root ownership, regular-file status, and trusted dispatcher directory, then applies that exact environment after the ordinary package.env stack; the existing metadata CPV/repository/compiler/ebuild checks remain mandatory. Focused runner, shell-syntax, and diff checks pass. The framework was republished and generation authority reactivated against the committed source. Commit `6c49082` records the change.

### 2026-09-21 exact cpio profile-use transaction

`app-arch/cpio-2.15::gentoo` completed a fresh exact generation-bound Clang IR training wave and exact profile-use transaction. The wave receipt is retained at `/var/lib/gentoo-optimization/generations/phase3-live-candidate-20260921-restored/cpio-wave-receipt.json`; merged profile and evidence are retained in the generation profile directory and `/var/cache/gentoo-optimization/pgo/`. Dispatcher publication bound CPV, repository, ebuild SHA-256, corrected fingerprint, generation, inventory, and active framework. The dedicated exact runner completed the `=app-arch/cpio-2.15::gentoo` pretend/source transaction with dispatcher handoff; independent receipt verification passed at `/var/lib/gentoo-optimization/reports/phase3-cpio-profile-use-restored-v2-receipt.json`. This is profile-use evidence only; no BOLT claim is made.

### 2026-09-21 exact gzip profile-use transaction

`app-arch/gzip-1.15::gentoo` was regenerated from the current live VDB identity, completed a fresh Clang IR generation wave, and passed the exact `=app-arch/gzip-1.15::gentoo` profile-use transaction through the dedicated dispatcher runner. The profile, merge evidence, dispatcher, wave receipt, and receipt verifier output bind the corrected fingerprint, repository, ebuild SHA-256, generation, inventory, and active framework. Independent receipt verification passed at `/var/lib/gentoo-optimization/reports/phase3-gzip-profile-use-restored-receipt.json`. This is profile-use evidence only; no BOLT claim is made.

### 2026-09-21 OpenSP profile-wave failure preserved

A fresh current-identity generation wave for `app-text/opensp-1.5.2-r10::gentoo` was attempted after binding the exact CPV to the existing `opensp-clang-cxx.conf` workaround. The first wave failed at C++ link with missing C++ runtime/RTTI symbols; the late exact package.env binding was added and the framework republished, but the regenerated wave still failed at the legacy libtool link boundary with `ld.lld --no-allow-shlib-undefined` reporting missing `operator new/delete`, `__cxa_begin_catch`, and C++ ABI symbols from `libosp.so`. The failed attempts and root-owned attempt receipts remain immutable. No profile was merged or published and no package-use success is claimed. The remaining remediation is to bind the actual libtool CXX/link-driver identity or explicit runtime library at the package build boundary, then regenerate rather than retrying unchanged.

### 2026-09-21 OpenSP compiler-driver boundary resolved; install compression failure preserved

The canonical LLVM alias hardening now selected `clang++-22` for OpenSP C++ compilation and all C++ link commands used the C++ driver with the runtime enabled. The fresh generation wave therefore passed the previous missing-runtime/RTTI failure. The transaction then failed later in Portage's `ecompress` install step with a segmentation fault while repairing compressed symlinks; no package merge or profile publication occurred. The wave receipt and attempt remain immutable. This is now a package/install-tool failure independent of the compiler-driver issue and requires a narrow compression/symlink remediation before OpenSP can become profile evidence.

### 2026-09-21 OpenSP compiler/install remediation succeeded; identity drift requires retraining

The package-local OpenSP policy and canonical `clang++-22` alias fix together produced a successful generation build and install transaction after disabling only the crashing documentation-compression pass. The merged LLVM profile payload was produced, but post-merge identity review found the live VDB ebuild SHA-256 had changed from the pre-wave fingerprint input (`1fbc3b03e9675704a68e0dba4fdf12919ad69aec36d74f1fe6ef881fd0dc682c`) to `eb345095f9bc5be24c5f1f95f5d506b9d7ace59267a9e6ca5f80bcb20c9b37a3`. Dispatcher publication was therefore refused; the payload and failed identity-binding attempt remain preserved, and OpenSP requires one current-identity retraining wave before profile-use evidence can be claimed.

### 2026-09-21 exact OpenSP profile-use transaction completed

After the current VDB identity was rematerialized, `app-text/opensp-1.5.2-r10::gentoo` completed a fresh exact Clang IR generation wave with the canonical `clang++-22` driver and package-local compression workaround. The post-wave ebuild SHA-256 remained `1fbc3b03e9675704a68e0dba4fdf12919ad69aec36d74f1fe6ef881fd0dc682c`; profile validation and dispatcher publication therefore passed. The dedicated exact runner completed `=app-text/opensp-1.5.2-r10::gentoo`, and independent receipt verification passed at `/var/lib/gentoo-optimization/reports/phase3-opensp-profile-use-restored-receipt.json`. This is Clang IR profile-use evidence only; no BOLT claim is made.

### 2026-09-21 yodl exact generation evidence and repository/ebuild drift
The exact `app-text/yodl-4.05.00` Clang-IR generation transaction completed with authenticated raw profile payloads and a merge receipt under `phase3-live-candidate-20260921-restored`. At consume-time, the production runner refused the dispatcher because the live package is sourced from repository `local` while the materialized fingerprint identity was generated from the `gentoo` ebuild (`906397af...`). The live `local` ebuild is independently hashed as `a0f2de7801f6f67d84dce622305896b6d44880338b517ad8645f01102cfc289e`; no profile-use admission was made. The raw payload, merge evidence, and refused identity-binding attempt remain preserved; yodl requires current-identity remediation/retraining before authoritative profile-use evidence.

### 2026-09-21 exact yodl profile-use transaction completed after live-repository remediation
The yodl raw profile was rematerialized against the live `local` repository ebuild identity (`a0f2de7801f6f67d84dce622305896b6d44880338b517ad8645f01102cfc289e`) rather than the stale `gentoo` identity. The corrected fingerprint is `6f294d9184c59cc7d21ee7e7904b42d8fccc93deaa0393e7dc07170bacf2dff8`; a fresh metadata sidecar and dispatcher were published through the active framework. The dedicated runner completed the exact `=app-text/yodl-4.05.00::local` transaction, and independent verification passed at `/var/lib/gentoo-optimization/reports/phase3-yodl-profile-use-restored-local-receipt.json`. This is Clang IR profile-use evidence only; no BOLT claim is made.

### 2026-09-21 — de-instrumentation residual narrowed to source-highlight ABI remediation

The authenticated de-instrumentation batch processed `dev-util/source-highlight-3.1.9-r2`, `media-sound/ncspot-1.3.4`, `media-video/rav1e-0.8.1`, and `x11-terms/alacritty-9999` under exact `--nodeps` transactions with the ABI guard active. The three latter CPVs completed without residual instrumented ELF records. `source-highlight` was rejected fail-closed because its clean `-O2 -pipe` replacement removed 11 established Boost/source-highlight exports (old provider 1339 exports, replacement 1352 with the old symbols missing); no merge was admitted and the marker `/var/lib/gentoo-optimization/state/deinstrument.pending` remains asserted. The fresh independent census recorded 8 instrumented ELF records in that one CPV and two separate non-ELF/readelf metadata failures. A package-local visibility/ABI remediation was attempted and reproduced the same guard rejection; the failure remains unresolved ABI evidence, not a guard bypass or terminal exclusion.
...
## 2026-09-22 post-cleanup live inventory candidate

A fresh live VDB inventory was regenerated after de-instrumentation and the source-highlight remediation. Candidate report `/var/lib/gentoo-optimization/reports/phase3-live-post-cleanup-20260922-candidate.json` has SHA-256 `f9c4c01ffa8f927b09b60a19b56e92d541c706a78491bca75b8179be24f370fc`, 1,307 CPVs, 684,782 owned paths, 80,079 owned-directory records, and 16,731 ELF records through the retained artifact census. The candidate remains non-authoritative because four runtime/spool directory records (`/etc/X11/xinit/xinitrc.d`, `/var/spool/cron/crontabs`, `/var/spool/cron/lastrun`, `/var/spool/cups/tmp`) did not retain stable live metadata across the review/generation boundary; no mutation policy or new framework authority was published from this candidate.

## 2026-09-22 root-authoritative inventory regeneration

The prior four unresolved directory records were caused by running the generator unprivileged: the generator intentionally converts metadata `stat(2)` failures into unresolved records, and the protected directories could not be stat'ed by the non-root process. Re-running the same producer as root against the same live VDB and reviewed directory evidence produced `/var/lib/gentoo-optimization/reports/phase3-live-post-cleanup-20260922-authoritative-candidate.json`, SHA-256 `17fb92f14dc2bd0d2740a7b63009b5ef5745e37fa7ffa3800104440c34b8b3b4`, with 1,307 CPVs, 684,782 owned paths, 80,079 directories, 16,731 ELF records, and **zero unresolved directories**. The independent frozen-inventory verifier passed. Mutation-policy publication remains pending because ten source-unavailable/lifecycle classification records still require exact evidence; no framework authority was inferred from inventory verification alone.

## 2026-09-22 source-unavailable mutation-policy review

The exact source/lifecycle review boundary was extended with an explicit, generation-bound `source-unavailable-review-v1` record rather than silently defaulting missing ebuilds to userspace. The reviewed record covers the ten CPVs whose historical ebuilds are no longer available in the configured repositories: nine receive an explicit userspace transaction disposition backed by their exact VDB `CONTENTS`, `BUILD_TIME`, `REPOSITORY`, and `INHERITED` evidence, while `sys-kernel/dracut-111-r1` remains an explicit `kernel-policy-exclusion`. The classifier refuses malformed or duplicate review rows, refuses a review row when an ebuild is actually available, and refuses any review that would demote concrete forbidden-artifact evidence. Regenerated classification reports 1,297 userspace decisions and 10 kernel-policy exclusions; the mutation-policy generator and independent verifier both pass for all 1,307 CPVs. This is candidate policy evidence; framework activation remains a separate authenticated transaction.

## 2026-09-22 post-cleanup candidate framework and generation authority

The corrected root-authoritative inventory, explicit source-unavailable policy review, successor lane reconciliation, and fresh VDB fingerprint inputs were materialized into candidate generation `phase3-live-candidate-20260922-post-cleanup-v2`. The lane reconciliation covers all 1,307 CPVs with 523 `not-applicable`, 238 unsupported/prebuilt, 509 Clang-IR, 18 Rust, 6 Go, 3 GCC, and 10 kernel-policy-exclusion records. Optimization-set generation passed with 1,232 userspace CPVs and 10 kernel-policy exclusions. Fresh exact VDB fingerprint inputs covered 536 PGO-lane records and materialized with 536 successes and zero failures.

The generated-policy tree was materialized and published through the root-owned framework installer. The authenticated install completed with `PASS: root-owned Phase 2 framework install verified`; the follow-up strict `--check` independently passed. Active framework is now `framework-6ce51479e0cd637a1db362941b658ae065be44979394b2c588646f26ffe73840`. Generation authorization was then activated and independently verified for `phase3-live-candidate-20260922-post-cleanup-v2` against inventory SHA-256 `0b0a6ecb7cf7012a53181e5b71657d6330518a622be51398de9ac0ce0623dacd`. No package wave was started in this boundary.
