...
- **Current execution frontier (supersedes the predecessor narrative below):** the live installation has already completed the jsonschema prerequisite historically: `dev-python/jsonschema-4.26.0` and `jsonschema-specifications-2025.9.1` are installed, and checkpoint `checkpoint-post-jsonschema-20260906T012300Z-v3` reached terminal `offline-restore-proven` after a real binary-only restore. The current non-frozen installed CPV count is 1,239. Historical Candidate A is source commit `18c030a4ac81574ed217f96cc6d32a8e2b52ba4e`; its authoritative boundary passed 93/93 top-level cases and 547 required subtests with zero required failures/skips. Commit `40e6b900c72c58a99aa96d1950b66e4c4fd7ec81` was an attempted Candidate-B freeze, invalidated by later verifier changes. Candidates C (`677277c`), D (`62c7484`), E (`30738bb`), and F (`b77f3e8`) are superseded by concrete historical-authority compatibility corrections, including one temporary source-edit defect caught before publication. The next action is Candidate G: run the complete authoritative boundary from the corrected retained-manifest logic, then publish detached 0/0/0 authorization. No new prerequisite replay is authorized; prerequisite preparation/mutation remains retired and fail-closed. No boot, kernel, EFI, initramfs, or bootloader mutation is permitted.
...
- **Project state:** active; Phase 0 and Phase 1 are complete and Phase 2 implementation is complete at 14/14. Historical Candidate A is clean; the former Candidate-B freeze was invalidated by later source changes. Candidates C through F are superseded by narrow historical-authority compatibility corrections; Candidate G is the next frozen authorization source. Authorization remains none until its detached index verifies 0/0/0. Phase 2 is scope-frozen: no new subsystem, optimization-policy axis, evidence category, or broad refactor may be added unless an existing required gate exposes a reproducible blocker that cannot be fixed within the current architecture.
