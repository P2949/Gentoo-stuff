...
- **Current execution frontier (supersedes the predecessor narrative below):** the live installation has already completed the jsonschema prerequisite historically: `dev-python/jsonschema-4.26.0` and `jsonschema-specifications-2025.9.1` are installed, and checkpoint `checkpoint-post-jsonschema-20260906T012300Z-v3` reached terminal `offline-restore-proven` after a real binary-only restore. The current non-frozen installed CPV count is 1,239. The exact clean framework source boundary is `262dd426210f2275c8002f43b5c8f4c3e42c62e8`; its root-owned bootstrap is published and verified, and Candidate A framework `framework-07a888137ca0ed6a54e2f1812c5600430552dd3ec4504f2f4884e11600fc1459` is installed and passes the strict framework check. Candidate A is not yet accepted, Candidate B and its detached index do not exist, and Phase 2 remains 7/14 and unauthorized. Later retry IDs remain immutable consumed history; `jsonschema-source-20260907T013000Z`, `...180000Z`, and `...190000Z` retain rollback/recovery state that must be reconciled before any further package mutation. No boot, kernel, EFI, initramfs, or bootloader mutation is permitted.
