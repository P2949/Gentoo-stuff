...
- **Current execution frontier (supersedes the predecessor narrative below):** the live installation has already completed the jsonschema prerequisite historically: `dev-python/jsonschema-4.26.0` and `jsonschema-specifications-2025.9.1` are installed, and checkpoint `checkpoint-post-jsonschema-20260906T012300Z-v3` reached terminal `offline-restore-proven` after a real binary-only restore. The current non-frozen installed CPV count is 1,239. The current source boundary is `4dae1c3`; the earlier Candidate-A framework/sample-PGO evidence from `e403813` is preserved historical evidence and must be rerun after the authorization-contract corrections. New prerequisite preparation/mutation is retired and fail-closed. Candidate A remains not finally accepted; Candidate B and its detached index do not exist. Phase 2 is 12/14 and unauthorized. No boot, kernel, EFI, initramfs, or bootloader mutation is permitted.
