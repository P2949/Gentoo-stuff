...
def main():
 ...
 out['coverage_pass']=not out['packages_missing_lane'] and not out['elf_missing_classification'] and not out['elf_missing_safety_review'];out['sha256']=hashlib.sha256(json.dumps(out,sort_keys=True,separators=(',',':')).encode()).hexdigest();json.dump(out,open(a.output,'w'),sort_keys=True,indent=2);open(a.output,'a').write('\n');print(json.dumps({k:out[k] for k in ('coverage_pass','package_count','elf_count','candidate_safety_records')}))
