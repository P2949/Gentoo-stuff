...
def read(args,p):
 try:return subprocess.run(['readelf',*args,p],text=True,stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,timeout=5).stdout
...
def main():
 ...
 for x in m['artifacts']:
  ...
  sec=read(['-S'],x['path']); syms=read(['-sW'],x['path']); text=any(' .text ' in (' '+l+' ') and 'PROGBITS' in l for l in sec.splitlines()); funcs=sum(1 for l in syms.splitlines() if ' FUNC ' in l and ' UND ' not in l); rows.append({'owner_cpv':x['owner_cpv'],'path':x['path'],'text_section':text,'defined_function_symbols':funcs,'state':'bolt-ready-pending-profile' if text and funcs else 'not-applicable','reason_code':None if text and funcs else ('missing-text-section' if not text else 'no-defined-function-symbols')})
 out={'record_type':'bolt-safety-review','schema_version':1,'source_sha256':m['sha256'],'records':sorted(rows,key=lambda z:z['path'])};out['counts']={'bolt-ready-pending-profile':sum(x['state']=='bolt-ready-pending-profile' for x in rows),'not-applicable':sum(x['state']=='not-applicable' for x in rows)};out['sha256']=hashlib.sha256(json.dumps(out,sort_keys=True,separators=(',',':')).encode()).hexdigest();json.dump(out,open(a.output,'w'),sort_keys=True,indent=2);open(a.output,'a').write('\n');print(out['counts'])
