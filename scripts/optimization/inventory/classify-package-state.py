...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--manifest',required=True);ap.add_argument('--census',required=True);ap.add_argument('--kernel-set',required=True);ap.add_argument('--output',required=True);a=ap.parse_args()
 m=json.load(open(a.manifest)); c=json.load(open(a.census)); owners={x['owner_cpv'] for x in c['artifacts'] if x['kind'] in ('regular','symlink') and x.get('elf')}; k=set(x.strip() for x in open(a.kernel_set) if x.strip()); rows=[]
 for cpv in m['cpvs']:
  if cpv in k: state,reason='kernel-policy-exclusion','kernel-policy-exclusion'
