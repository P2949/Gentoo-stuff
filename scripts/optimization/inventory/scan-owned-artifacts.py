 ...
 for cat in sorted(os.listdir(a.vdb)):
  ...
  for pf in sorted(os.listdir(cd)):
   ...
   for line in open(c,errors='replace'):
    ...
    seen.add(key); kind='missing'; size=None; mode=None; elf=None; interp=None
    try:
     ...
     if stat.S_ISLNK(st.st_mode): kind='symlink'
      ...
      if h[:4]==b'\x7fELF':
       elf={'class':h[4],'data':h[5],'type':int.from_bytes(h[16:18],'little'),'machine':int.from_bytes(h[18:20],'little')}
     else: kind='other'
    except OSError: pass
    rows.append({'owner_cpv':owner,'path':path,'kind':kind,'size':size,'mode':mode,'elf':elf})
