...
def main():
    ...
    result={'generation_id':a.generation_id,'inventory_id':a.generation_id+'-v1','owned_directories':outdirs,'owned_paths':sorted(set(paths),key=lambda x:(x['path'],x['owner_cpv'])),'packages':sorted(packages,key=lambda x:x['cpv']),'record_type':'frozen-inventory','schema_version':2}
